 ...
 * sent; for example, a POST request). Sending a large request body to a server
 * after a request has been rejected for inappropriate headers would be
 * inefficient. To have a server check the request's headers, a client must send
...
/**
 * SINCE HTTP/1.1 The requested resource is available only through a proxy, the
 ...
 * however, future requests should still use the original URI. In contrast to
 ...
 */
export const UNSUPPORTED_MEDIA_TYPE_STATUS_CODE = 415;
