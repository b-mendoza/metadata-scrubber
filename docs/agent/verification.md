...
- Use only the commands each service documents; do not invent or guess build, lint, or test commands or flags. If a documented command is missing or broken, say so instead of improvising a substitute.
- Never report a check as passing unless you ran it against the current state of the change and saw it pass. Report results as they are — including failures and warnings — rather than summarizing them into a cleaner story.
...
- Do not hand-edit generated or tooling-managed files such as lockfiles (`pnpm-lock.yaml`, `go.sum`). This applies to any generated artifact: change the source or generator and regenerate, letting the managing tool produce the diff.
